# Claude Code Project Instructions — winapi-util-kotlin

## Project Overview

This is **winapi-util-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`winapi-util`](https://crates.io/crates/winapi-util).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/winapi-util/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/winapi-util>
